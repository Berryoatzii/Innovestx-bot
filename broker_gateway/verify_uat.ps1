...
Write-Host 'Running read-only UAT verification. No order will be sent.' -ForegroundColor Yellow
& $pythonExe (Join-Path $gatewayDir 'verify_uat.py')
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
Write-Host 'UAT gateway and account read are available.' -ForegroundColor Green
